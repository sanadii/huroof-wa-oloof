...
export function projectRoom(roomId: string, room: CanonicalRoom, members: CanonicalMember[], role: MemberRole, uid?: string, now = Date.now()) {
  ...
  const active = members.filter((member) => member.active); const participants = active.filter((member) => member.role !== 'audience'); const self = uid ? active.find((member) => member.uid === uid) : undefined; const match = deriveMatch(room.game); const canStart = allReady(room, participants); const questionVisible = ['QUESTION_READING', 'FIRST_ANSWER', 'OPPONENT_CHANCE', 'QUESTION_FAILED'].includes(room.game.lifecycle); const mediaVisible = role === 'host' || (role === 'audience' && room.config.showQuestionOnAudience !== false); const publicQuestion = room.activeQuestion && questionVisible ? { headerAr: room.activeQuestion.headerAr, promptAr: room.activeQuestion.promptAr, ...(mediaVisible && room.activeQuestion.media ? { media: room.activeQuestion.media } : {}), ...(room.game.lifecycle === 'QUESTION_FAILED' ? { revealedAnswer: room.activeQuestion.canonicalAnswer } : {}) } : undefined; const winner = room.buzzWinner ? { displayName: room.buzzWinner.displayName, team: room.buzzWinner.team, method: room.buzzWinner.method } : undefined;
  ...
  if (role === 'host') { if (room.activeQuestion) projection.question = { headerAr: room.activeQuestion.headerAr, promptAr: room.activeQuestion.promptAr, ...(questionVisible && room.activeQuestion.media ? { media: room.activeQuestion.media } : {}), primaryAnswer: room.activeQuestion.canonicalAnswer, acceptedAnswers: room.activeQuestion.acceptedAnswers, sources: room.activeQuestion.sources ?? [] }; if (room.game.correction) projection.correction = { ...room.game.correction, priorOwner: room.game.board?.cells.find((cell) => cell.id === room.game.correction?.cellId)?.owner }; }
...
export function reduceIntent(room: CanonicalRoom, member: CanonicalMember, intent: GameIntent, now: number, question?: CanonicalQuestion, members: CanonicalMember[] = [member], boardLetters?: string[], surpriseLetter?: string, manualParticipantId?: string): { room: CanonicalRoom; member: CanonicalMember; targetMember?: CanonicalMember; eventType: string } {
  ...
  if (charades && (intent.type === 'START_MATCH' || intent.type === 'START_NEXT_ROUND')) {
    ...
    return { room: { ...room, revision: room.revision + 1, game, activeQuestion: question, timer: { deadlineMs: now + room.config.questionSeconds * 1000, buzzOpen: true }, buzzWinner: undefined, questionCursor: room.questionCursor + 1 }, member, eventType: intent.type };
  }
  ...
  if (!charades && (intent.type === 'RETRY_CELL' || intent.type === 'RETURN_CELL') && room.game.lifecycle === 'QUESTION_FAILED') {
    return { room: { ...room, revision: room.revision + 1, game: reduceGame(room.game, { type: 'RETURN_CELL' }), activeQuestion: undefined, timer: undefined, buzzWinner: undefined }, member, eventType: intent.type };
  }
  if (intent.type === 'END_WITHOUT_WINNER') {
    ...
    return { room: { ...room, revision: room.revision + 1, game: { ...room.game, lifecycle: 'MATCH_COMPLETE', answeringTeam: undefined, activeCellId: undefined, endedWithoutWinner: true, contentHold: undefined }, activeQuestion: undefined, timer: undefined, buzzWinner: undefined }, member, eventType: intent.type };
  }
  ...
  if (intent.type === 'SELECT_CELL') {
    ...
    return { room: { ...room, revision: room.revision + 1, game, timer: { deadlineMs: now + room.config.questionSeconds * 1000, buzzOpen: true }, activeQuestion: question, buzzWinner: undefined, questionCursor: room.questionCursor + 1 }, member, eventType: intent.type };
  }
  let event: GameEvent;
  switch (intent.type) {
  ...
  if ((intent.type === 'LETTER_REVEALED' || intent.type === 'RETRY_CELL') && game.lifecycle === 'QUESTION_READING') { timer = { deadlineMs: now + room.config.questionSeconds * 1000, buzzOpen: true }; buzzWinner = undefined; }
  if (intent.type === 'OPEN_QUESTION') {
    if (game.lifecycle !== 'QUESTION_READING') throw new Error('question-not-ready');
    if (!timer?.buzzOpen) { timer = { deadlineMs: now + room.config.questionSeconds * 1000, buzzOpen: true }; buzzWinner = undefined; }
  }
  if (intent.type === 'JUDGE_INCORRECT' && game.lifecycle === 'OPPONENT_CHANCE') { timer = { deadlineMs: now + room.config.opponentSeconds * 1000, buzzOpen: true }; buzzWinner = undefined; }
  ...
  if (intent.type === 'RESUME' && room.timer?.remainingMs !== undefined) timer = { deadlineMs: now + room.timer.remainingMs, buzzOpen: Boolean(room.timer.pausedBuzzOpen) };
  if (['JUDGE_CORRECT', 'RETURN_CELL', 'AWARD_CELL', 'CHECK_PATH', 'START_NEXT_ROUND'].includes(intent.type)) timer = undefined;
  return { room: { ...room, revision: room.revision + 1, game, timer, activeQuestion, buzzWinner, questionCursor: room.questionCursor + (questionSelectedAtReveal ? 1 : 0) }, member, eventType: intent.type };
}
