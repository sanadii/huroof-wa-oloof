...
type Tile = { row: number; column: number; id?: string; labelAr?: string; shape?: string; hex?: string; bitmask?: number; missing?: boolean };
type Option = { id: string; labelAr: string; shape: string; hex: string; bitmask?: number };
...
type WordSearchStimulus = { rows: number; columns: number; grid: readonly (readonly string[])[]; clueAr: string; reveal?: { paths?: { start: { row: number; column: number }; end: { row: number; column: number } }[] } };
...
const kindTitle: Record<ChallengeProjection["kind"], string> = { missing_tile: "الجزء المفقود", qatar_map: "لوكيشن قطر", memory: "لون الصورة", navigation: "وجّه صاحبك", word_search: "صيد المختفي" };
...
function RuleCard({ challenge }: { challenge: ChallengeProjection }) {
  const tileRule = (challenge.stimulus as { rule?: "vertical_mirror" | "latin" | "xor_row_16_dot" } | undefined)?.rule;
  const duration = challenge.kind === "missing_tile"
    ? `${challenge.timing.answerSeconds ?? 0} ثانية`
  ...
  const tileGuidance = tileRule === "vertical_mirror" ? "اختر القطعة المطابقة لنظيرتها في الانعكاس العمودي. يمكنك تغيير اختيارك قبل التأكيد." : tileRule === "latin" ? "اختر القطعة التي تجعل كل صف وعمود بعناصر مختلفة. يمكنك تغيير اختيارك قبل التأكيد." : tileRule === "xor_row_16_dot" ? "في كل صف: تضيء النقطة في القطعة اليمنى إذا ظهرت في قطعة واحدة فقط من القطعتين السابقتين (XOR). يمكنك تغيير اختيارك قبل التأكيد." : "اختر القطعة التي تكمل القاعدة. يمكنك تغيير اختيارك قبل التأكيد.";
  return <section className="challenge-rules" aria-label="قواعد التحدي"><h2>{kindTitle[challenge.kind]}</h2><p>{challenge.kind === "missing_tile" ? tileGuidance : challenge.kind === "qatar_map" ? "اقرأ السؤال ثم اختر الحروف أو رتّبها. لا يحتاج الترتيب إلى سحب." : challenge.kind === "memory" ? "احفظ الألوان أولاً؛ سيظهر سؤال التذكّر بعد إخفاء اللوحة." : "يلزم هاتف خاص للدليل. يوجّه اللاعب بالكلام، بينما يتحرك اللاعب أو وكيل المضيف خطوة واحدة."}</p><p><strong>المدة: {duration}</strong></p></section>;
}

function TileGlyph({ shape, hex, bitmask, label, selected }: { shape?: string; hex?: string; bitmask?: number; label?: string; selected?: boolean }) {
  ...
  const dotText = bitmask === undefined ? undefined : Array.from({ length: 16 }, (_value, index) => `النقطة ${index + 1}: ${((bitmask >>> index) & 1) === 1 ? "تشغيل" : "إيقاف"}`).join("، ");
  if (normalizedShape === "dot_matrix_4x4") return <svg className={`${className} challenge-tile--dot-matrix`} viewBox="0 0 100 100" preserveAspectRatio="xMidYMid meet" style={{ height: "72%", width: "72%" } as CSSProperties} aria-label={[label, dotText].filter(Boolean).join(". ")}>
    {Array.from({ length: 16 }, (_value, index) => <circle key={index} cx={17 + (index % 4) * 22} cy={17 + Math.floor(index / 4) * 22} r="7" fill={((bitmask ?? 0) >>> index) & 1 ? (hex ?? "#1d3447") : "transparent"} stroke="currentColor" strokeWidth="2" />)}
  </svg>;
...
function TileChallenge({ challenge, canSubmit, onSubmit, staticOptions = false }: { challenge: ChallengeProjection; canSubmit: boolean; onSubmit: (answers: string[]) => void; staticOptions?: boolean }) {
  const data = challenge.stimulus as { rows?: number; columns?: number; cells?: Tile[]; options?: Option[]; rule?: "vertical_mirror" | "latin" | "xor_row_16_dot"; promptAr?: string; reveal?: { correctOptionId?: string } } | undefined;
  ...
  return <section className="challenge-stimulus challenge-stimulus--tile" aria-label="لغز الخلية الناقصة">
    ...
    <div className="challenge-tile-grid" style={{ gridTemplateColumns: `repeat(${data.columns ?? 3}, 1fr)` }} data-testid="tile-grid">
      {data.cells.map((cell) => { const preview = cell.missing ? options.find((option) => option.id === (reveal ?? draft)) : undefined; return <div className={`challenge-tile-cell${cell.missing ? " is-missing" : ""}${preview ? " is-preview" : ""}${reveal && cell.missing ? " is-solution" : ""}`} key={`${cell.row}:${cell.column}`}>{preview ? <TileGlyph hex={preview.hex} bitmask={preview.bitmask} label={`${reveal ? "الحل" : "معاينة"} ${preview.labelAr}`} shape={preview.shape} selected={Boolean(reveal)} /> : cell.missing ? <span aria-label="خلية ناقصة">؟</span> : <TileGlyph hex={cell.hex} bitmask={cell.bitmask} label={cell.labelAr} shape={cell.shape} />}</div>; })}
    </div>
    {reveal ? <p className="challenge-explanation" role="status">الحل: الخيار {options.find((item) => item.id === reveal)?.labelAr ?? "الصحيح"}. {data.rule === "vertical_mirror" ? "القطعة تقابل نظيرتها بالانعكاس العمودي." : data.rule === "xor_row_16_dot" ? "في كل صف: تضيء النقطة في القطعة اليمنى إذا ظهرت في قطعة واحدة فقط من القطعتين السابقتين (XOR)." : "كل صف وعمود يكتمل بعنصر مختلف."}</p> : null}
    {(canSubmit || staticOptions) && !reveal ? <><fieldset className="challenge-options" disabled={staticOptions}><legend>{staticOptions ? "الخيارات" : "اختر قطعة واحدة"}</legend>{options.map((option, index) => { const sourceLetter = /^[أبجد]$/u.test(option.labelAr); return <button type="button" className={`challenge-option${draft === option.id ? " is-selected" : ""}`} aria-pressed={draft === option.id} key={option.id} onClick={() => setDraft(option.id)}>{sourceLetter ? <span className="challenge-option__letter">{option.labelAr}</span> : <><span className="challenge-option__letter">{["أ", "ب", "ج", "د"][index]}</span><span>{option.labelAr}</span></>}<TileGlyph hex={option.hex} bitmask={option.bitmask} shape={option.shape} /></button>; })}</fieldset>{canSubmit ? <button className="button button--primary" disabled={!draft} onClick={() => draft && onSubmit([draft])} type="button">تأكيد الإجابة</button> : null}</> : null}
  </section>;
}
...
function WordSearchChallenge({ challenge, canSubmit, onSubmit }: { challenge: ChallengeProjection; canSubmit: boolean; onSubmit: (start: { row: number; column: number }, end: { row: number; column: number }) => void }) {
  const data = challenge.stimulus as WordSearchStimulus | undefined;
  const [start, setStart] = useState<{ row: number; column: number }>();
  const [end, setEnd] = useState<{ row: number; column: number }>();
  const key = `${challenge.occurrence}:${challenge.assignmentGeneration}:${challenge.stage}`;
  useEffect(() => { setStart(undefined); setEnd(undefined); }, [key]);
  if (!data) return <RuleCard challenge={challenge} />;
  const select = (point: { row: number; column: number }) => {
    if (!canSubmit || data.reveal?.paths) return;
    if (!start || end) { setStart(point); setEnd(undefined); return; }
    setEnd(point);
  };
  const selected = (point: { row: number; column: number }) => point.row === start?.row && point.column === start?.column ? " is-start" : point.row === end?.row && point.column === end?.column ? " is-end" : "";
  const revealEndpoints = new Set((data.reveal?.paths ?? []).flatMap((path) => [`${path.start.row}:${path.start.column}`, `${path.end.row}:${path.end.column}`]));
  return <section className="challenge-stimulus challenge-stimulus--word-search" aria-label="شبكة صيد الكلمة"><p className="challenge-prompt">{data.clueAr}</p><p className="challenge-word-search-instruction" id="word-search-instruction">اختر حرف البداية ثم حرف النهاية في خط مستقيم. ترتيب اللمس أو لوحة المفاتيح لا يغيّر اتجاه الشبكة.</p><div className="challenge-word-search-scroll"><div aria-describedby="word-search-instruction" aria-label={`شبكة ${data.rows} في ${data.columns}، اتجاه الأعمدة من اليسار إلى اليمين`} className="challenge-word-search-grid" data-testid="word-search-grid" role="grid" style={{ gridTemplateColumns: `repeat(${data.columns}, minmax(2.75rem, 1fr))` }}>{data.grid.flatMap((row, rowIndex) => row.map((letter, column) => {
    const point = { row: rowIndex, column }, endpoint = revealEndpoints.has(`${rowIndex}:${column}`), position = start?.row === rowIndex && start.column === column ? "البداية المختارة" : end?.row === rowIndex && end.column === column ? "النهاية المختارة" : endpoint ? "طرف مسار الحل" : "غير مختار";
    return <button aria-label={`الصف ${rowIndex + 1}، العمود ${column + 1}، الحرف ${letter}، ${position}`} aria-pressed={Boolean(selected(point))} className={`challenge-word-search-cell${selected(point)}${endpoint ? " is-reveal" : ""}`} data-column={column} data-row={rowIndex} disabled={!canSubmit || Boolean(data.reveal?.paths)} key={`${rowIndex}:${column}`} onClick={() => select(point)} role="gridcell" type="button">{letter}</button>;
  }))}</div></div>{data.reveal?.paths ? <p className="challenge-explanation" role="status">تظهر أطراف المسار الصحيح بعد إظهار المضيف للحل.</p> : <><p className="challenge-word-search-draft" role="status">{start && end ? `المسار المختار: من الصف ${start.row + 1}، العمود ${start.column + 1} إلى الصف ${end.row + 1}، العمود ${end.column + 1}.` : start ? `تم اختيار البداية: الصف ${start.row + 1}، العمود ${start.column + 1}. اختر النهاية.` : "اختر حرف البداية."}</p>{canSubmit ? <button className="button button--primary" disabled={!start || !end} onClick={() => start && end && onSubmit(start, end)} type="button">تأكيد المسار</button> : null}</>}</section>;
}
...
export function ChallengeStage({ challenge, role, serverTime, connection, authoritative, teams, scores, members = [], error, labelledColours = false, onIntent, onTerminalCorrection, onDeadline }: ChallengeStageProps) {
  ...
  const submitWordSearch = (start: { row: number; column: number }, end: { row: number; column: number }) => void send("CHALLENGE_SUBMIT", { start, end });
