...
export async function validateV31CategoryArtifacts(): Promise<string[]> {
  ...
  const schema = JSON.parse(await readFile(join(ROOT, schemaPath), 'utf8')); const AjvConstructor = Ajv2020 as unknown as new (options: Record<string, unknown>) => { compile: (input: object) => ((value: unknown) => boolean) & { errors?: Array<{ instancePath?: string }> } }; const validateSchema = new AjvConstructor({ allErrors: true, strict: false }).compile(schema);
