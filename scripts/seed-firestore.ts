...
export async function buildDemoDraftSeed() {
  ...
  return { releaseId: RELEASE_ID, documentRootSha256, letters: letters.size, documents: drafts.map((draft) => ({ path: `releases/${RELEASE_ID}/questions/${draft.id}`, data: { ...draft, demoUnreviewed: true } })) };
}
async function main() {
