...
Verified baseline: `/admin/categories` has a short published detail but no direct question link. `/admin/questions` already pages published questions by server-side `categoryId`, but has no URL deep-link. Question detail has text/answer and verified <=1 MB media preview; the release snapshot contains no `difficulty` field. Published content remains immutable, and `adminCategoryMetadata` is not player-facing. Security review found no bypass in source and specified scope, allowlist, media and stale-response boundaries. Browser observed guest access only. Exact next authorized action: implement [T39.1](T39-ADMIN-CATEGORY-INSPECTION.md#t391--published-category--questions-inspection) in the isolated clean worktree. T39.2 awaits user edit semantics and database review; no production content or schema changes have been made.
