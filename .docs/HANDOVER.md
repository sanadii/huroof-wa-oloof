...
T39.1 and T39.2 are root-accepted in the isolated `codex/admin-category-detail-20260924` branch. Category rows now link to category and server-filtered question views; the question detail shows available prompt, answer, aliases, media/answer media, points and difficulty when stored. The active release has no stored difficulty values, so the page says so. Category edits save a release-bound correction draft with immutable revisions and an internal note; published gameplay remains unchanged. Security and independent technical Gate passed; the UI test is 23/23, Functions tests 66 passed with 3 fixture skips, and the focused Firebase emulator transaction test is 1/1. T39.3 is the exact next authorized action: publish the scoped PR, deploy targeted backend and frontend, read back the production version, then verify with a signed-in admin on desktop/mobile. The observed browser had guest access only, so authenticated live inspection remains unverified. See [T39 plan](T39-ADMIN-CATEGORY-INSPECTION.md).
