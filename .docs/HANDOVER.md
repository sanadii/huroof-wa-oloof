...
## T40 category question-type filter — 2026-09-24

The user requested category filtering by question-side type. The host setup category picker is the provisional target; the TV display has no selector, and the user has been asked to clarify the surface. [T40 contract](T40-QUESTION-TYPE-FILTER.md) records implementation, privacy and rollback. M40-A metadata is root-accepted; M40-B host UI is implemented with independent Gate PASS, focused app/index tests, builds, and Firestore Rules denial passing. Signed-in desktop/mobile use with real metadata remains unverified. The exact T38 index is prepared but no production sidecar has been written: standard Admin SDK credentials and a fresh active-pointer readback are required. Automatic approval review rejected reuse of the Firebase CLI refresh token as an ADC workaround. Existing T39 and mixed local work remain untouched.
...
# T39 admin category handover — 2026-09-24

T39.1 and T39.2 are root-accepted and published through merged [PR #4](https://github.com/sanadii/huroof-wa-oloof/pull/4). Category rows now link to category and server-filtered question views; question detail shows available prompt, answer, aliases, media/answer media, points and difficulty when stored. The active release has no stored difficulty values, so the page says so. Category edits save a release-bound correction draft with immutable revisions and an internal note; published gameplay remains unchanged. Security and independent technical Gate passed; UI 23/23, Functions 66 passed with 3 fixture skips, and focused Firebase emulator transaction 1/1. Production Vercel deployment `dpl_3VRbwmGSwSjTVfaokUDu9xBMaDoH` is Ready at the canonical alias; all four targeted admin Functions read back ACTIVE, with the draft gate enabled and general mutation gate unset. The Firebase CLI's only post-deploy failure was Artifact Registry cleanup-policy setup, left unchanged. T39.3 remains open solely for authenticated desktop/mobile and media behavior checks; the available browser session was a guest. See [T39 plan](T39-ADMIN-CATEGORY-INSPECTION.md).

The 2026-09-24 inline-media follow-up is live through merged [PR #6](https://github.com/sanadii/huroof-wa-oloof/pull/6), Ready Vercel deployment `dpl_6vN2SvagwHNNBCrpxAtyW23auWc6`, and the canonical production alias. Published images now render within their question/answer section; videos open in native inline controls on click. An authenticated local Firebase session verified a real image question, paired question/answer goal videos, actual playback, and narrow-layout behavior. Admin UI tests passed 25/25, lint and build passed, and independent Gate found no blockers. The existing media callable and production content data were not changed. The production browser tab was a guest, so authenticated production visual inspection is the only remaining T39.3 check; see [T39 evidence](T39-ADMIN-CATEGORY-INSPECTION.md#t393--integration-and-production).

Current authorized work (2026-09-24): [T39.4](T39-ADMIN-CATEGORY-INSPECTION.md#t394--category-question-inspection-workflow-authorized-2026-09-24) adds published category identity and ordered question navigation, then a shared release-scoped reviewed check that saves before advancing. Database and Security reviews are resolved in the canonical plan; no release, editorial approval, or gameplay data mutation is authorized. Next action: implement T39.4 in the isolated worktree, verify focused UI/Functions/emulator/security behavior, then root/Gate review before T39.5 production rollout. The previous production authenticated-browser gap remains open until a signed-in session is available.

T39.4/T39.5 update (2026-09-24): [PR #8](https://github.com/sanadii/huroof-wa-oloof/pull/8) merged as `95b06078371a6a537362a849406e7522d04b27ee`; Ready Vercel production `dpl_5SvtoytMWQS1WojhVerdQAf7DczB` serves the updated admin UI and the goal-category cover (HTTP 200, 49,680 bytes). Targeted Node 22 Firebase Functions deployed backend-first with the new review gate off, then `adminGetSession` and `adminMarkPublishedQuestionInspected` redeployed with only that gate on. A signed-in local admin using the production backend saw the real category title/image, first/second previous/next links, inline video, and enabled review button for the unchanged active release ID. No real question was marked during release verification. Functions 69 passed/3 fixture skips, UI 28/28, Firebase emulator/Rules 1/1, builds/typecheck/lint/diff check and independent Gate PASS; root independently reran UI 28/28. Firebase CLI reported successful function operations, followed by its unchanged Artifact Registry cleanup-policy error. Production-origin authenticated mark persistence/automatic advance and human mobile visual approval remain open. [Exact evidence and rollback](T39-ADMIN-CATEGORY-INSPECTION.md#t395--integrated-release-and-production-readback). Four unrelated UI files became modified in the original implementation worktree after publication; they were preserved and excluded from this docs-only evidence checkout.
