...
Local 5188 correction: that port had served the older admin-review checkout, so its category list had no selector. It now serves the T40 checkout while the older checkout and its four uncommitted game-view edits remain untouched. The live admin catalog is the later `t37m-media-cc919463d962a644fefb20b5d05ed8f8` release, not the T38 B2 predecessor. For local Vite development only, a loopback-only endpoint supplies a metadata-only preview index derived from the reviewed T38 B2 index plus the T37.M verified additive change of 39 image questions in `tahadani-011`. The preview is bound to the verified T37.M root `7ef9b9fe0aad1b6945bd3d2b476da3f831626085174990643fa6240db68b771c`, 115,636 questions and 530 categories; a root/release/category/total mismatch disables the fallback. This is not a production index publication or a claim that the T37.M successor was independently rescanned for the T40 classifier. The generated preview file remains ignored under `output/t40-question-type-20260924/T37M-INDEX.json` and is served only by local Vite on loopback.

Signed-in Chrome readback on `http://localhost:5188/admin/categories` showed the selector enabled against T37.M. Selecting `فيديو` completed all authorized category pages and showed the matching `goals-2026` category. Focused admin and preview tests passed (32), as did typecheck, lint and client build. Production remains governed by T40.5's create-only publication and fresh-release checks.
